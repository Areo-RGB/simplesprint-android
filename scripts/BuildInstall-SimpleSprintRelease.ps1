...
if ($clientSerials.Count -gt 0) {
    Invoke-Step -Name "Assemble release APK (client profile)" -Action {
    ...
    foreach ($deviceId in $clientSerials) {
        Invoke-Step -Name "Install client APK on $deviceId" -Action {
            powershell -ExecutionPolicy Bypass -File .\scripts\Install-SimpleSprintRelease.ps1 -DeviceId $deviceId | Out-Host
        }
    }
}

Invoke-Step -Name "Assemble release APK (tablet host profile)" -Action {
    .\gradlew.bat :app:assembleRelease -PtabletAlwaysHost=true | Out-Host
}

Invoke-Step -Name "Install tablet-host APK on $TabletHostSerial" -Action {
    powershell -ExecutionPolicy Bypass -File .\scripts\Install-SimpleSprintRelease.ps1 -DeviceId $TabletHostSerial | Out-Host
}

Write-Host "`nDone. Release build installed and launched. tablet-host=$TabletHostSerial"
