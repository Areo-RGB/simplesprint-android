...
class RoiFrameDiffer {
    ...
    fun scoreLumaPlane(
        ...
        val squareSizePx = max(1, (roiWidth * width).toInt())
        val roiCenterYPx = height / 2
        val startX = max(0, roiCenterPx - (squareSizePx / 2))
        val endX = min(width, startX + squareSizePx)
        val startY = max(0, roiCenterYPx - (squareSizePx / 2))
        val endY = min(height, startY + squareSizePx)
