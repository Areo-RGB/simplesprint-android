...
class MainActivity : ComponentActivity(), ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    private lateinit var nsdServiceCoordinator: NsdServiceCoordinator
    ...
    private var pendingHostDiscoveryHintJob: Job? = null
    ...
    private val isForcedTabletHost: Boolean
        get() = shouldForceTabletHostMode(
            tabletAlwaysHostFlag = BuildConfig.TABLET_ALWAYS_HOST,
            isTabletRoleChoiceDevice = isTabletRoleChoiceDevice,
        )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nsdServiceCoordinator = NsdServiceCoordinator(
            backend = com.paul.simplesprint.core.services.AndroidNsdBackend(this),
        ...
        setContent {
            com.paul.simplesprint.ui.theme.SprintSyncTheme {
                SprintSyncApp(
                            ...
                            try {
                                connectionsManager.startHosting(
                                    ...
                                    if (result.isSuccess) {
                                        ...
                                        startNsdHosting(errorPrefix = "host advertise")
                                    }
                        ...
                        requestPermissionsIfNeeded(PermissionScope.NETWORK_ONLY) {
                            ...
                            try {
                                connectionsManager.startHosting(
                                    ...
                                    if (result.isSuccess) {
                                        startNsdHosting(errorPrefix = "display host advertise")
                                    }
                                    result.exceptionOrNull()?.let { error ->
                    ...
                    onReconnectClient = {
                        if (isForcedTabletHost) {
                            appendEvent("Tablet host mode is enforced for this APK.")
                            return@SprintSyncApp
                        }
                        if (uiState.value.setupBusy) return@SprintSyncApp
        ...
        onBackPressedDispatcher.addCallback(
            ...
            object : OnBackPressedCallback(true) {
                override fun handleOnBackPressed() {
                    ...
                    val shouldReturnToLobby = shouldHandleSystemBackToLobby(
                        isForcedTabletHost = isForcedTabletHost,
                        operatingMode = state.operatingMode,
                        stage = state.stage,
                    )
                    if (shouldReturnToLobby) {
                        raceSessionController.stopMonitoring()
                        appendEvent("Back pressed: returned to lobby")
                        syncControllerSummaries()
                        return
                    }
                    isEnabled = false
        ...
        val autoStartRole = resolveAutoStartRole(
            isForcedTabletHost = isForcedTabletHost,
            isTabletRoleChoiceDevice = isTabletRoleChoiceDevice,
            configuredAutoStartRole = BuildConfig.AUTO_START_ROLE,
        )
        if (autoStartRole != "none") {
            lifecycleScope.launch {
                delay(250)
                if (autoStartRole == "host") {
                    setSetupBusy(true)
                    requestPermissionsIfNeeded(PermissionScope.NETWORK_ONLY) {
                        clearDisplayRelayReconnectionState()
                        displayDiscoveryActive = false
                        ...
                        connectionsManager.startHosting(
                            ...
                            if (result.isSuccess) {
                                startNsdHosting(errorPrefix = "host auto-start advertise")
                            }
                            result.exceptionOrNull()?.let { error ->
    ...
    private fun startNsdHosting(errorPrefix: String) {
        nsdServiceCoordinator.startHosting(port = BuildConfig.TCP_HOST_PORT) { result ->
            result.exceptionOrNull()?.let { error ->
    ...
    private fun stopNsdHosting() {
        nsdServiceCoordinator.stopHosting()
    }

    private fun startNsdDiscovery(errorPrefix: String) {
        stopNsdDiscovery()
    ...
    private fun stopNsdDiscovery() {
        ...
        nsdServiceCoordinator.stopDiscovery()
    }

    private fun startMissingHostHintTimer() {
        cancelMissingHostHintTimer()
        pendingHostDiscoveryHintJob = lifecycleScope.launch {
    ...
    private fun cancelMissingHostHintTimer() {
        pendingHostDiscoveryHintJob?.cancel()
        pendingHostDiscoveryHintJob = null
    }
    ...
    private fun applyRequestedOrientationForMode(mode: SessionOperatingMode) {
        val targetOrientation = requestedOrientationForMode(
            mode = mode,
            isTabletHostDevice = isTabletRoleChoiceDevice,
        )
...
internal fun shouldHandleSystemBackToLobby(
    isForcedTabletHost: Boolean,
    operatingMode: SessionOperatingMode,
    stage: SessionStage,
): Boolean {
    return isForcedTabletHost &&
...
internal fun resolveAutoStartRole(
    ...
    configuredAutoStartRole: String,
): String {
    if (isForcedTabletHost) {
        return "host"
    }
    if (!isTabletRoleChoiceDevice) {
        return "client"
    }
    return configuredAutoStartRole
}
...
internal fun requestedOrientationForMode(mode: SessionOperatingMode, isTabletHostDevice: Boolean = false): Int {
    ...
    if (mode == SessionOperatingMode.NETWORK_RACE && isTabletHostDevice) {
